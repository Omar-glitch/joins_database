...
SELECT * FROM olympics.dbo.games_competitor gc
WHERE gc.games_id IN (
	SELECT TOP 10 g.id FROM olympic	s.dbo.games g ORDER BY g.games_year DESC;
);

SELECT nr.region_name FROM noc_region nr 
WHERE nr.id not in(
 SELECT DISTINCT(nr.id) FROM(
  SELECT TOP 10 id  FROM games g
  order by games_year DESC 
 ) gf INNER JOIN games_competitor gc on gf.id=gc.games_id
 INNER JOIN person p on gc.person_id=p.id
 INNER JOIN 
  (SELECT pr.person_id,MIN(pr.region_id) region_id FROM person_region pr group by pr.person_id) prd on p.id=prd.person_id 
 INNER JOIN noc_region nr on prd.region_id=nr.id
)
